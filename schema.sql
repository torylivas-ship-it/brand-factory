...
-- ── ops_orders (BFN Ops — separate product, not a content-pack tier) ──
...
-- ── outreach_leads (cold-SMS outreach tracking — pre-customer prospects, not accounts) ──
...
-- Admin-only table: never read or written by the frontend, only by backend
-- admin routes using the service key. No anon/authenticated policies at all.
